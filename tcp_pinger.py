...
class TcPinger:
    ...
    def _ping(self):
        ...
        if resp_packet is None:
            ...
            self._parse_resp_packet(resp_packet, spent_time)
    ...
    def _parse_resp_packet(self, packet: Packet, spent_time: float):
    ...
    def _close_connection(self):
        sr1(IP(dst=self._dst_ip) / TCP(sport=self._src_port, dport=self._dst_port, flags='R'),
            timeout=1,
            verbose=0)

    def start_ping(self):
        print(f"start ping {self._dst_ip}:{self._dst_port}:")
    ...
    def _print_line(self, is_received: bool, ans_time=0.0, message=""):
        if is_received:
