...
class TcPinger:
    ...
    def _process_ping(self, dst_port: int):
        ...
        if resp_packet is None:
            ...
            self._parse_resp_packet(resp_packet, spent_time, dst_port=dst_port)
            self.statistics[dst_port].times.append(spent_time)
            self.statistics[dst_port].received_packs += 1
    ...
    def _parse_resp_packet(self, packet: Packet, spent_time: float, dst_port: int):
        ...
        if flag == 0x12:
            ...
            self._close_connection(dst_port)
            print(f"received unexpected flags combination in response packet: {hex(flag)}")
    ...
    def _ping(self):
        for port in self._dst_ports:
            self._process_ping(dst_port=port)
            self.statistics[port].sent_packs += 1
